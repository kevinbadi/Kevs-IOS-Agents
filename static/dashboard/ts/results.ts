...
type ResultsPlatform = 'all' | 'tiktok' | 'instagram' | 'linkedin';
...
interface LinkedInConnectFunnel {
    total: number;
    sent: number;
    remaining: number;
    lists: Array<{ name: string; total: number; sent: number; remaining: number }>;
}
...
interface ResultsSnapshot {
    ...
    workflows: Array<Totals & { pluginId: string; taskType: string; label: string; platform: string }>;
    devices: Array<Totals & { udid: string; name: string }>;
    linkedinConnect?: LinkedInConnectFunnel;
}

const rangeEl = document.querySelector<HTMLElement>('#results-range')!;
const dailyEl = document.querySelector<HTMLElement>('#results-daily')!;
const chartMeta = document.querySelector<HTMLElement>('#results-chart-meta')!;
const workflowsEl = document.querySelector<HTMLElement>('#results-workflows')!;
const devicesEl = document.querySelector<HTMLElement>('#results-devices')!;
const todayCaption = document.querySelector<HTMLElement>('#results-today-caption')!;
const rangeHeading = document.querySelector<HTMLElement>('#results-range-heading')!;
const linkedinPanel = document.querySelector<HTMLElement>('#results-linkedin')!;
const linkedinStats = document.querySelector<HTMLElement>('#results-linkedin-stats')!;
const linkedinCaption = document.querySelector<HTMLElement>('#results-linkedin-caption')!;
const linkedinBar = document.querySelector<HTMLElement>('#results-linkedin-bar')!;
const platformButtons = Array.from(document.querySelectorAll<HTMLButtonElement>('.platform-switch-btn[data-platform]'));
const rangeButtons = Array.from(document.querySelectorAll<HTMLButtonElement>('.results-range-btn[data-days]'));

const params = new URLSearchParams(location.search);
let platform: ResultsPlatform = isPlatform(params.get('platform')) ? params.get('platform') as ResultsPlatform : 'all';
let days = Number.parseInt(params.get('days') ?? '14', 10);
if (![7, 14, 30].includes(days)) days = 14;

function isPlatform(value: string | null): value is ResultsPlatform {
    return value === 'all' || value === 'tiktok' || value === 'instagram' || value === 'linkedin';
}

function sentLabel(current: ResultsPlatform): string {
    if (current === 'instagram') return 'DMs';
    if (current === 'linkedin') return 'Connects';
    return 'Sent';
}

function formatDay(isoDate: string, style: 'short' | 'day' = 'short'): string {
    const [year, month, day] = isoDate.split('-').map(Number);
    const date = new Date(year, month - 1, day);
    if (style === 'day') return String(day);
...
function heroStats(totals: Totals, current: ResultsPlatform): Array<[number, string]> {
    const cards: Array<[number, string]> = [[totals.likes, 'Likes'], [totals.comments, 'Comments']];
    if (current === 'all' || current === 'tiktok') cards.push([totals.saves, 'Saves']);
    if (current === 'all' || current === 'instagram' || current === 'linkedin') {
        cards.push([totals.sent, sentLabel(current)]);
    }
    if (current === 'tiktok') cards.push([totals.posts, 'Posts']);
    cards.push([totals.runs, 'Runs']);
    return cards;
}

function compactStats(totals: Totals, current: ResultsPlatform): Array<[number, string]> {
    return heroStats(totals, current).filter(([value, label]) => value > 0 || label === 'Runs');
}

function renderStats(target: HTMLElement, totals: Totals): void {
    target.className = 'results-stats';
    target.replaceChildren(...heroStats(totals, platform).map(([value, label]) => statCard(value, label)));
}

function renderLinkedIn(funnel: LinkedInConnectFunnel | undefined): void {
    ...
    if (!funnel || funnel.total === 0) {
        ...
        linkedinStats.replaceChildren(
            statCard(0, 'Leads'),
            statCard(0, 'Sent'),
            statCard(0, 'Remaining'),
        );
    ...
    linkedinStats.className = 'results-stats';
    linkedinStats.replaceChildren(
        statCard(funnel.total, 'Leads'),
        statCard(funnel.sent, 'Sent'),
        statCard(funnel.remaining, 'Remaining'),
    );
    const fill = linkedinBar.querySelector('span');
    if (fill) fill.style.width = `${Math.min(100, Math.round((funnel.sent / funnel.total) * 100))}%`;
    linkedinBar.hidden = false;
}

function renderDaily(daysRows: ResultsSnapshot['daily']): void {
    if (!daysRows.length) {
        dailyEl.hidden = true;
    ...
    dailyEl.replaceChildren(...daysRows.map((row, index) => {
        ...
        if (row.runs === 0) bar.classList.add('is-empty');
...
function renderList(
    target: HTMLElement,
    items: Array<Totals & { title: string; meta: string }>,
    empty: string,
): void {
    if (!items.length) {
...
function todayLine(snapshot: ResultsSnapshot): string {
...
async function load(): Promise<void> {
    ...
    const body = await response.json() as ResultsSnapshot & { error?: string };
